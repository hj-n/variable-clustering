...
def hdbscan_scorer(X, labels, scorer):
	...
	optimizer = BayesianOptimization(hdbscan_scorer_inner, pbounds, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
...
def dbscan_scorer(X, labels, scorer):
	...
	optimizer = BayesianOptimization(f=dbscan_scorer_inner, pbounds=pbounds, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
...
def kmeans_scorer(X, labels, scorer):
	...
	optimizer = BayesianOptimization(f=kmeans_scorer_inner, pbounds=pbound, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
...
def xmeans_scorer(X, labels, scorer):
	...
	optimizer = BayesianOptimization(f=xmeans_scorer_inner, pbounds=pbound, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
...
def birch_scorer(X, labels, scorer):
	...
	optimizer = BayesianOptimization(f=birch_scorer_inner, pbounds=pbound, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
...
def agglo_scorer(X, labels, scorer, linkage):
	...
	n_candidates  = len(np.unique(labels))
	pbound = {'n_clusters': (2, 3 * n_candidates)}
	optimizer = BayesianOptimization(f=agglo_scorer_inner, pbounds=pbound, verbose=0, random_state=1, allow_duplicate_points=True)
	optimizer.maximize()
